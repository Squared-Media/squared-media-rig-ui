...
class VIEW3D_PT_ui_Main(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row.enabled = True
        row.operator(
            "squaredmedia.copy_debug_info", text=version_str, emboss=True, icon="INFO"
        )
        ...
        col.scale_y = 1.5
        col.prop(
            preferences.preferences, "rigTab", text="Default Import Option", expand=True
        )
        ...
        if (
            ...
            and context.active_object.get("rig_id") == rigID
            and get_material_object(context.active_object)
        ):
            layout.separator()
