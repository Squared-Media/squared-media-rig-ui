...
from .list import VIEW3D_PT_rig_list, PopulateList
bl_info = {
...
class RigItem(bpy.types.PropertyGroup):
    ...
    rigID: bpy.props.StringProperty(name="Rig ID")

class ListProperties(bpy.types.PropertyGroup):
    rig_list : bpy.props.CollectionProperty(type=RigItem)
    rig_index: bpy.props.IntProperty(name="Index", default=0)
...
def register():
    ...
    bpy.types.Scene.rig_props = bpy.props.PointerProperty(type=ListProperties)
